...
var _RestaurantDetail_instances, initializeDOM_fn, initializeEventListeners_fn, handleSubmit_fn, _RestaurantFilter_instances, renderFilterCategory_fn, renderFilterSort_fn, _RestaurantForm_instances, renderTitle_fn, renderForm_fn, handleSubmit_fn2, getInputValue_fn, getFormData_fn, resetFormData_fn, _RestaurantList_instances, initializeDOM_fn2, _restaurants, _currentFilter, _selectedRestaurant, _App_instances, initializeCompoenents_fn, createHeader_fn, createMain_fn, createRestaurantNavBar_fn, createRestaurantFilter_fn, createRestaurantList_fn, createSubmitFormBottomSheet_fn, createDetailBottomSheet_fn;
(function polyfill() {
...
class RestaurantDetail {
  constructor({ onToggleFavorite, onDelete, onClose }) {
    ...
    __publicField(this, "$categoryImg", document.createElement("img"));
    __publicField(this, "$name", document.createElement("h3"));
    __publicField(this, "$distance", document.createElement("span"));
    __publicField(this, "$description", document.createElement("p"));
    __publicField(this, "$link", document.createElement("a"));
    __publicField(this, "$favoriteButton", document.createElement("button"));
    __publicField(this, "$favoriteImg", document.createElement("img"));
    __publicField(this, "$closeButton");
    this.onToggleFavorite = onToggleFavorite;
    this.onDelete = onDelete;
    this.onClose = onClose;
    __privateMethod(this, _RestaurantDetail_instances, initializeDOM_fn).call(this);
    __privateMethod(this, _RestaurantDetail_instances, initializeEventListeners_fn).call(this);
  }
  updateDetailContent(restaurant) {
...
initializeEventListeners_fn = function() {
  ...
  this.$form.addEventListener(
    EVENT_TYPES.submit,
    __privateMethod(this, _RestaurantDetail_instances, handleSubmit_fn).bind(this)
  );
};
...
class RestaurantFilter {
  constructor({ onFilterChange }) {
    ...
    __publicField(this, "$filterContainer", document.createElement("section"));
...
renderForm_fn = function() {
  ...
  $form.addEventListener(EVENT_TYPES.submit, __privateMethod(this, _RestaurantForm_instances, handleSubmit_fn2).bind(this));
  return $form;
};
...
class RestaurantList {
  constructor(restaurantList, { onToggleFavorite, onOpenDetail }) {
    ...
    __publicField(this, "onToggleFavorite");
    __publicField(this, "onOpenDetail");
    __publicField(this, "$listSection", document.createElement("section"));
    __publicField(this, "$list", document.createElement("ul"));
    this.restaurantList = restaurantList;
    this.onToggleFavorite = onToggleFavorite;
    this.onOpenDetail = onOpenDetail;
    __privateMethod(this, _RestaurantList_instances, initializeDOM_fn2).call(this);
  }
  render() {
    this.$list.innerHTML = "";
...
function createRestaurantNavBar(options) {
  return new RestaurantNavBar(options);
}
function createRestaurantFilter(options) {
  return new RestaurantFilter(options);
}
function createRestaurantList(restaurantList, options) {
  return new RestaurantList(restaurantList, options);
}
function createBottomSheetBase(options) {
  return new BottomSheetBase(options);
}
function createRestaurantForm(options) {
  return new RestaurantForm(options);
}
function createRestaurantDetail(options) {
  return new RestaurantDetail(options);
}
function handleOpenSubmitForm($submitFormBottomSheet) {
  if ($submitFormBottomSheet) $submitFormBottomSheet.open();
}
function handleOpenDetail(restaurantId, store, $detailBottomSheet) {
  store.updateSelectedRestaurant(restaurantId);
  if ($detailBottomSheet) $detailBottomSheet.open();
}
function handleCloseBottomSheet($bottomSheet) {
  if ($bottomSheet) $bottomSheet.close();
}
function handleTabChange(tabType, store, $restaurantFilter) {
  store.setFilter({
...
function handleFilterChange(filterType, store) {
  store.setFilter({
    ...store.state.currentFilter,
    filterType
  });
}
function handleToggleFavorite(restaurantId, store) {
  store.toggleFavorite(restaurantId);
}
function handleSubmitForm(newRestaurantInfo, store, $submitFormBottomSheet) {
  store.addRestaurant(newRestaurantInfo);
  if ($submitFormBottomSheet) $submitFormBottomSheet.close();
}
function handleDeleteRestaurant(restaurantId, store) {
...
const _RestaurantStorage = class _RestaurantStorage {
  constructor() {
    __publicField(this, "storage", new LocalStorage());
  }
  getRestaurants() {
    return this.storage.get(_RestaurantStorage.RESTAURANTS_KEY) ?? [];
  }
  setRestaurants(restaurants) {
    return this.storage.set(
      _RestaurantStorage.RESTAURANTS_KEY,
      restaurants
    );
  }
  removeRestaurants() {
    this.storage.remove(_RestaurantStorage.RESTAURANTS_KEY);
  }
  clearRestaurants() {
    this.storage.clear();
  }
};
__publicField(_RestaurantStorage, "RESTAURANTS_KEY", "restaurants");
let RestaurantStorage = _RestaurantStorage;
const generateUUID = () => {
...
class RestaurantStore extends Observable {
  constructor() {
    super();
    __publicField(this, "storage", new RestaurantStorage());
    __privateAdd(this, _restaurants, []);
    __privateAdd(this, _currentFilter, DEFAULT_FILTER_OPTIONS);
    __privateAdd(this, _selectedRestaurant, null);
    const savedRestaurants = this.storage.getRestaurants();
    if (savedRestaurants) {
      __privateSet(this, _restaurants, savedRestaurants);
  ...
  get state() {
    return {
      ...
      selectedRestaurant: __privateGet(this, _selectedRestaurant) ?? DEFAULT_RESTAURANT
    };
  }
  addRestaurant(restaurant) {
    ...
    this.storage.setRestaurants(__privateGet(this, _restaurants));
    this.notifyListeners();
  }
...
const getFilteredRestaurants = (restaurants, options = DEFAULT_FILTER_OPTIONS) => {
  const {
    tabType,
...
function setupRestaurantListSubscription(store, restaurantList) {
...
function setupRestaurantDetailSubscription(store, restaurantDetail) {
...
function setupSubscriptions(store, components) {
  const unsubscribes = [];
...
class App {
  constructor() {
    ...
    __publicField(this, "store", new RestaurantStore());
    __publicField(this, "$body", document.body);
    __publicField(this, "$restaurantFilter");
    __publicField(this, "$restaurantList");
    __publicField(this, "$submitFormBottomSheet");
    __publicField(this, "$detailBottomSheet");
    __publicField(this, "$restaurantDetail");
    __privateMethod(this, _App_instances, initializeCompoenents_fn).call(this);
...
initializeCompoenents_fn = function() {
  const $header = __privateMethod(this, _App_instances, createHeader_fn).call(this);
  const $main = __privateMethod(this, _App_instances, createMain_fn).call(this);
  render([$header, $main], this.$body);
  if (!$main) return;
  render(
    [
      __privateMethod(this, _App_instances, createRestaurantNavBar_fn).call(this),
      __privateMethod(this, _App_instances, createRestaurantFilter_fn).call(this),
      __privateMethod(this, _App_instances, createRestaurantList_fn).call(this),
      __privateMethod(this, _App_instances, createSubmitFormBottomSheet_fn).call(this),
      __privateMethod(this, _App_instances, createDetailBottomSheet_fn).call(this)
    ],
    $main
  );
};
...
createMain_fn = function() {
  return document.createElement("main");
};
createRestaurantNavBar_fn = function() {
  return createRestaurantNavBar({
    onTabChange: (tabType) => handleTabChange(tabType, this.store, this.$restaurantFilter)
  }).render();
};
createRestaurantFilter_fn = function() {
  this.$restaurantFilter = createRestaurantFilter({
    onFilterChange: (filterType) => handleFilterChange(filterType, this.store)
  });
  return this.$restaurantFilter.render();
};
createRestaurantList_fn = function() {
  ...
  this.$restaurantList = createRestaurantList(restaurantList, {
    onToggleFavorite: (restaurantId) => handleToggleFavorite(restaurantId, this.store),
    onOpenDetail: (restaurantId) => handleOpenDetail(restaurantId, this.store, this.$detailBottomSheet)
  });
  return this.$restaurantList.render();
};
createSubmitFormBottomSheet_fn = function() {
  const $restaurantForm = createRestaurantForm({
    title: "새로운 음식점",
    onSubmit: (newRestaurantInfo) => handleSubmitForm(
      newRestaurantInfo,
      this.store,
...
createDetailBottomSheet_fn = function() {
  this.$restaurantDetail = createRestaurantDetail({
    onToggleFavorite: (restaurantId) => handleToggleFavorite(restaurantId, this.store),
    onDelete: (restaurantId) => handleDeleteRestaurant(restaurantId, this.store),
